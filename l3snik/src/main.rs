...
async fn main() {
    ...
    // into logging task
    let (tx_sender_error_info, _tx_receiver_error_info) = mpsc::channel(5);
    ...
    let tx_sender_error_info_arc = Arc::new(Mutex::new(tx_sender_error_info));
    
    tokio::spawn(instantiate_proxy(address, tx_sender_proxy_info, tx_sender_error_info_arc.clone()));

    unsafe {
        std::env::set_var("WGPU_POWER_PREF", "high");
    }

    gui::L3snikGui::start(tx_receiver_proxy_info, tx_sender_error_info_arc.clone());
} 
