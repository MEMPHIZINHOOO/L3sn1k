...
pub async fn instantiate_proxy(arg: SocketAddr, tx_proxy: Sender<ProxyEvent>, tx_log: Sender<anyhow::Error>) -> Result<String, ()> 
{
...
// #[warning(dead_code)]
pub fn check_init_ca_dir() -> Result<(), String> {
    // we read the dir
    ...
    if  Path::new(std::env::var("CA_DIR").expect("path does not exist isn't supposed to happen").as_str()).exists() {
        if let Ok(directory) = fs::read_dir(std::env::var("CA_DIR").unwrap_or(CA_DEFAULT_DIR.to_string())){
            for entry in directory {

                let entry: DirEntry = entry.expect("wasn't able to grab the dir entry");
            ...
            if checks.len() != 0 {
                return Err("failed to match the necessary files for proxy execution".to_string());
            }
            return Ok(());  
        }
